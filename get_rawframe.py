...
#存放视频的地址
videos_src_path = r'E:\dataset\custom'
#videos_src_path = r'C:\dataset\custom_ava'

#存放图片的地址
videos_save_path = r'E:\dataset\videos_frame'
...
def parse_args():
    ...
    parser.add_argument('--time', default=3, help='input video duration seconds')
    args = parser.parse_args()
    return args

def get_frames(video_name, output = False):
    # 获取每个视频的名称
    each_video_name, _ = video_name.split('.')
    # 创建目录，来保存图片帧
    if os.path.exists(args.output + r'/' + each_video_name) == False and output == True:
        os.mkdir(args.output + r'/' + each_video_name)
    # 获取保存图片的完整路径，每个视频的图片帧存在以视频名为文件名的文件夹中
    each_video_save_full_path = os.path.join(args.output, each_video_name) + r'/'
    # 获取每个视频的完整路径
    ...
    # print(frame_count)
    if frame_count <= args.fps * args.time:
        print(video_name + " not enough, frame: " + str(frame_count))

if __name__ == '__main__':
    args = parse_args()
    if os.path.isfile(args.input):
        get_frames(os.path.basename(args.input),can_output)
    else:
        # 返回videos_src_path路径下包含的文件或文件夹名字的列表（所有视频的文件名），按字母顺序排序
        videos = os.listdir(args.input)
        for each_video in videos:
            get_frames(each_video,can_output)
